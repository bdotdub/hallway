...
if (lconfig.ijod_backend === "file") {
  ...
  var backendmod;
  if (lconfig.s3) {
    logger.warn("No ijod_backend specified. Using ijod-s3 as there seems to be S3 config info available.");
    backendmod = require('ijod-s3');
  } else {
    logger.warn("No ijod_backend specified. Using ijod-mem.");
    backendmod = require('ijod-mem');
  }
  backend = new backendmod.backend();
}
...
exports.getPars = function(basePath, options, cbDone) {
  ...
  dal.query(sql, binds, function(err, rows) {
    ...
    var ret = {};
    var ndx = {}; // need a place to stash the variants temporarily
    var now = Date.now();
    ...
    rows.forEach(function(row) {
      ...
      var id;
      if (row.at > now) {
        // last nib is the variant, rest is prefix of real idr
        id = row.idr.substr(0,31);
        to = ndx;
      }else{
        id = row.idr;
        to = ret;
      ...
      if (!to[id]) to[id] = [];
      ndxr(to, id, row.par.substr(0,8));
      ndxr(to, id, row.par.substr(8,8));
      ndxr(to, id, row.par.substr(16,8));
      ndxr(to, id, row.par.substr(24,8));
    });
    logger.debug("got ",Object.keys(ret).length,"idrs and",
      Object.keys(ndx).length, "extras");
    // merge all other extra pars up to the main idr
    Object.keys(ret).forEach(function(idr) {
      lutil.addAll(ret[idr], ndx[idr.substr(0,32)]);
    });
    cbDone(null, ret);
  });
};
  ...
  memcache.get(hash, function(error, result) {
    ...
    if (error || result === "NOT_STORED" || result === null || !js) {
      dal.query("SELECT path, offset, len FROM Entries WHERE idr = x? LIMIT 1",
        ...
        backend.get(rows[0].path, rows[0].offset, buf.len, function (err, buf) {
